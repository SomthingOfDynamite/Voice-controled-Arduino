...
void readSerialCommands() {
    ...
    // LED 
    if (cmd == "LED_ON") { led_state = true; led_mode = 0; }
    else if (cmd == "LED_OFF") { led_state = false; digitalWrite(led_pin, LOW); }
    else if (cmd == "LED_SOLID") { led_mode = 0; }
    else if (cmd == "LED_FAST") { led_mode = 1; }
    else if (cmd == "LED_SLOW") { led_mode = 2; }

    // LCD 
    else if (cmd == "LCD_ON") { lcd_state = true; lcd_mode = 0; }
    else if (cmd == "LCD_OFF") { lcd_state = false; lcd.clear(); }
    else if (cmd == "LCD_WELCOME") { lcd_mode = 0; }
    else if (cmd == "LCD_OK") { lcd_mode = 1; }
    else if (cmd == "LCD_RESET") { lcd_mode = 2; }

    // MOTOR
    else if (cmd == "MOTOR_ON") { motor_state = true; motor_mode = 0; }
    else if (cmd == "MOTOR_OFF") { motor_state = false; analogWrite(motor_pin, 0); }
    else if (cmd == "MOTOR_SLOW") { motor_mode = 0; }
    else if (cmd == "MOTOR_MED") { motor_mode = 1; }
    else if (cmd == "MOTOR_FAST") { motor_mode = 2; }

    // SERVO
    else if (cmd == "SERVO_ON") { servo_state = true; servo_mode = 1; }
    else if (cmd == "SERVO_OFF") { servo_state = false; servo.write(0); }
    else if (cmd == "SERVO_45") { servo_mode = 0; }
    else if (cmd == "SERVO_90") { servo_mode = 1; }
    else if (cmd == "SERVO_180") { servo_mode = 2; }
}
